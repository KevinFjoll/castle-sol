...
contract PieceContract is ERC1155, Ownable {
  ...
  constructor(uint16[4] memory _puzzlesPerTier, uint8 _rowCount, uint8 _columnCount)
    ERC1155(
    ...
    for (uint8 tier = 1; tier <= puzzlesPerTier.length; tier++) {
      for (uint8 row = 1; row <= rowCount; row++) {
        for (uint8 col = 1; col <= columnCount; col++) {
          _mint(mintTo, calcTokenId(tier, row, col), puzzlesPerTier[tier-1], "");
        }
      }
    }
   ...
   */
  function calcTokenId(uint8 tier, uint8 row, uint8 column) public view returns (uint256 _tokenId) {
    return
      1 +
   ...
   */
  function getTokenIdsOfTier(uint8 tier) public view returns (uint256[] memory _tokenIds)
  {
    ...
    for (uint8 row = 1; row <= rowCount; row++) {
      for (uint8 col = 1; col <= columnCount; col++) {
        ids[uint256(row-1) * uint256(rowCount) + uint256(col-1)] = calcTokenId(tier, row, col);
      }
    }
    return ids;
  }
